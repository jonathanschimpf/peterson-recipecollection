...
export const load: PageServerLoad = async () => {
    try {
        ...
        // FILTER ONLY "DESSERTS" CATEGORY
        const desserts: Recipe[] = data.filter(
            (recipe) => recipe.category.trim() === "Desserts"
        );

        console.log("Filtered Desserts Data:", desserts); // DEBUG

        return { desserts };
    } catch (error) {
        console.error("ERROR LOADING RECIPE DATA:", error);
        return { desserts: [] }; // RETURN EMPTY ARRAY IF JSON LOAD FAILS
    }
};
