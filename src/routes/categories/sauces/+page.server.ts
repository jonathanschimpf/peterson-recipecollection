...
export const load: PageServerLoad = async () => {
    try {
        ...
        // FILTER ONLY "SAUCES" CATEGORY
        const sauces: Recipe[] = data.filter(
            (recipe) => recipe.category.trim() === "Sauces"
        );

        console.log("Filtered Sauces Data:", sauces); // DEBUG

        return { sauces };
    } catch (error) {
        console.error("ERROR LOADING RECIPE DATA:", error);
        return { sauces: [] }; // RETURN EMPTY ARRAY IF JSON LOAD FAILS
    }
};
