...
export const load: PageServerLoad = async () => {
    try {
        ...
        // FILTER ONLY "CHRISTMAS RECIPES" CATEGORY
        const christmasRecipes: Recipe[] = data.filter(
            (recipe) => recipe.category.trim() === "Christmas Recipes"
        );

        console.log("Filtered Christmas Recipes Data:", christmasRecipes); // DEBUG

        return { christmasRecipes };
    } catch (error) {
        console.error("ERROR LOADING RECIPE DATA:", error);
        return { christmasRecipes: [] }; // RETURN EMPTY ARRAY IF JSON LOAD FAILS
    }
};
