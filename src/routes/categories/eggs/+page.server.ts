...
export const load: PageServerLoad = async () => {
    try {
        ...
        // FILTER ONLY "EGGS" CATEGORY
        const eggs: Recipe[] = data.filter(
            (recipe) => recipe.category.trim() === "Eggs"
        );

        console.log("Filtered Eggs Data:", eggs); // DEBUG

        return { eggs };
    } catch (error) {
        console.error("ERROR LOADING RECIPE DATA:", error);
        return { eggs: [] }; // RETURN EMPTY ARRAY IF JSON LOAD FAILS
    }
};
