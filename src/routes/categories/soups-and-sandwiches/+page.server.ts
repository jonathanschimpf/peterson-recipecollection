...
export const load: PageServerLoad = async () => {
    try {
        ...
        // FILTER ONLY "SOUPS AND SANDWICHES" CATEGORY
        const soupsAndSandwiches: Recipe[] = data.filter(
            (recipe) => recipe.category.trim() === "Soups and Sandwiches"
        );

        console.log("Filtered Soups and Sandwiches Data:", soupsAndSandwiches); // DEBUG

        return { soupsAndSandwiches };
    } catch (error) {
        console.error("ERROR LOADING RECIPE DATA:", error);
        return { soupsAndSandwiches: [] }; // RETURN EMPTY ARRAY IF JSON LOAD FAILS
    }
};
