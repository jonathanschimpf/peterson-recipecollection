...
export const load: PageServerLoad = async () => {
    try {
        ...
        // FILTER ONLY "POULTRY" CATEGORY
        const poultry: Recipe[] = data.filter(
            (recipe) => recipe.category.trim() === "Poultry"
        );

        console.log("Filtered Poultry Data:", poultry); // DEBUG

        return { poultry };
    } catch (error) {
        console.error("ERROR LOADING RECIPE DATA:", error);
        return { poultry: [] }; // RETURN EMPTY ARRAY IF JSON LOAD FAILS
    }
};
